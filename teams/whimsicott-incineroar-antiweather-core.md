...
| 2026-09-09 | Archived at the M-B → M-C regulation rollover, per explicit user instruction that old teams "can be marked as legacy." Status: Draft → Archived. Nothing else in this file was touched, including its known item error (Staraptor's "Staraptorite" is not in the Champions item pool) — left wrong, uncorrected, as a historical record; not authorized to fix | User instruction, `vgc-regulation-transition` skill step 6 |
