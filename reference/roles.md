...
`championsLegal: false`** (originally verified 2026-09-09 under M-B;
re-verified 2026-09-09 again after the M-B → M-C rollover via `dex legal
--item` against the freshly re-vendored item list — same verdict, both still
absent). This is not a vendoring gap in the sense a missing species is — the
tool's own note is explicit that a `false` here means genuinely unavailable
right now, not merely un-vendored — but the pool is per-regulation and
reverts, so re-check again at the next rollover rather than trusting this
past M-C. **Practically: in current Champions, a legal Choice item means
Choice Scarf specifically** — the ×1.5 *damage* half of the source's
...
**One item class the source names is currently unavailable in its
entirety, and is marked as such directly in the table above rather than
excluded from it:**

- **Assault Vest** — the source lists it under "Defensive items" alongside
  Focus Sash, explicitly noting it "cannot run status moves." Originally
  verified 2026-09-09 under M-B via `dex legal --item "Assault Vest"`:
  `championsLegal: false`; re-verified 2026-09-09 again after the M-B → M-C
  rollover against the freshly re-vendored item list — same verdict, still
  absent. It is absent from the vendored Champions item pool entirely, not
  merely missing a vendoring update — the same tool call that confirmed
  Focus Sash, Life Orb, Leftovers and Sitrus Berry are all legal returned
  `false` specifically for this one item. Its row above is not cut for
  that: the guidance about what it does for a role is evergreen even though
  the item itself is sitting out this regulation too. Re-check again at the
  next rollover.

**Terrain seeds are back.** The other item class marked unavailable in this
file's earlier verification (Electric Seed, Grassy Seed, Misty Seed, Psychic
Seed — the source's "Seeds (activate on terrain)" Defensive-items class) all
returned `championsLegal: true` on re-check after the M-B → M-C rollover
(2026-09-09, `dex legal --item`). Their row in the table above no longer
carries an availability mark and reads as an ordinary, currently-legal item
class — this is exactly the case the "guidance is never deleted for current
unavailability" rule exists for: the class's guidance survived M-B intact
and needed no rewriting now that the items are legal again.
...
| 2026-09-09 | Regulation-transition step 5e: re-verified this file's three live Champions-departure markers against the freshly re-vendored M-C item pool via `dex legal --item`. Choice Band and Choice Specs are still `championsLegal: false` — restamped their marker and footnote 1 from M-B to M-C (date unchanged, 2026-09-09, since the rollover landed the same day as the original check). Assault Vest is also still `championsLegal: false` — same restamp, table row and "Two item classes..." explainer paragraph (now "One item class..." since terrain seeds dropped out of it). The four terrain seeds (Electric, Grassy, Misty, Psychic) all came back `championsLegal: true` — removed their marker from the table row entirely (it now reads as an ordinary, currently-legal item class) and rewrote the former "Terrain seeds" bullet under "item classes... currently unavailable" into a "Terrain seeds are back" note instead of deleting it, recording that the class's guidance survived M-B unmarked and needed no changes now that the items are legal — exactly the outcome the "never delete guidance for current unavailability" rule was written to produce. `npm test` — 371/371, unchanged | `node tools/dex/cli.js legal --item "Choice Band"` / `"Choice Specs"` / `"Assault Vest"` / `"Electric Seed"` / `"Grassy Seed"` / `"Misty Seed"` / `"Psychic Seed"`; `reference/regulation.md`'s M-C stamp block |
