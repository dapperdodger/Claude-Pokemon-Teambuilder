...
the starting-point step and the archetype step of the `vgc-team-building`
skill — the checklist says "agree the archetype with the user," and this is
the file that says what the archetypes are, what each one requires, and how
each one fails.
...
`reference/format-knowledge.md`. The first three are now written; only
