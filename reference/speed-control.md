...
`reference/format-knowledge.md`. The first two are now written; only
...
| Turn-order changers — Trick Room, Tailwind | Field-wide effects that change turn order itself: Tailwind raises Speed side-wide for a fixed window, Trick Room reverses the Speed comparison entirely (`reference/mechanics.md`, "Speed calculation" and "Priority & turn order") | A team slot plus the setup turn; Trick Room reverses turn order, so both effects running together make your Tailwind boost a liability (see `reference/archetypes.md`'s Tailwind and Trick Room sections for requirements and failure modes) |
...
- **It costs Stat Points that did not go to bulk or power.** The same SP
  ...
  securing a KO — see `reference/team-evaluation.md` for the fuller
  inherent-vs-relative-strength trade-off this sits inside.
